...
class _CashFlowStatementScreenState extends State<CashFlowStatementScreen> {
  late CashFlowStatement _report;

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    try {
      _report = ModalRoute.of(context)!.settings.arguments as CashFlowStatement;
    } catch (e) {
      _report = CashFlowStatement.initial();
    ...
    ReportHelpDialog.show(
      ...
      sections: [
        const HelpSection(
          ...
          content:
              'Shows how cash moves in and out of your business across Operating, Investing, and Financing activities.',
        ),
        const HelpSection(
          title: 'Before Generating',
          items: [
            HelpItem(text: '- Upload all relevant documents'),
            HelpItem(text: '- Ensure documents are "Posted"'),
            HelpItem(text: '- Generate Profit & Loss report FIRST'),
          ],
        ),
        const HelpSection(
          title: 'Dependencies',
          content:
              'Cash Flow depends on Net Income from the Profit & Loss report. Always generate Profit & Loss first!',
        ),
        const HelpSection(
  ...
  Widget _buildBody(BuildContext context, ReportState state) {
    ...
    if (loadedReport is! CashFlowStatement) {
      return const Center(
        child: Text(
  ...
  Widget _buildReportHeader(CashFlowStatement report) {
    final startDate =
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(
          bottom: BorderSide(color: Colors.green.shade900, width: 3),
        ),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildSectionTotal(ReportSection section) {
    // Calculate grand total from only the visible (non-zero) items across all groups
    double visibleGrandTotal = 0.0;
    for (var group in section.groups) {
      final visibleItems = group.line_items
          .where((item) => item.amount.abs() >= 0.005)
          .toList();
      visibleGrandTotal += visibleItems.fold(
        0.0,
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Text(
            section.section_title != 'Cash Balance'
                ? 'Total Cash Flow'
                : 'Total Cash Balance',
            style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold),
          ),
          Text(
            _formatCurrency(visibleGrandTotal),
            style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold),
          ),
        ],
      ),
            ...
            children: [
              ...
              Padding(
                ...
                child: Text(
                  ...
                  style: TextStyle(
                    fontSize: 13,
                    color: _getAmountColor(item.amount),
                    fontWeight: FontWeight.w500,
                  ),
                ),
              ),
  ...
  Widget _buildGroupSubtotal(ReportGroup group) {
    // Calculate subtotal from only the visible (non-zero) items
    final visibleItems = group.line_items
        .where((item) => item.amount.abs() >= 0.005)
        .toList();
    final visibleSubtotal = visibleItems.fold(
      0.0,
      (sum, item) => sum + item.amount,
    );
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            _formatCurrency(visibleSubtotal),
  ...
  Widget _buildFinancialSummary(CashFlowStatement report) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: Colors.green.shade50,
      ...
      child: Column(
        ...
        children: [
          ...
          _buildMetricRow(
            ...
            report.total_operating_cash_flow,
          ),
          _buildMetricRow(
            'Investing Activities',
            report.total_investing_cash_flow,
          ),
          _buildMetricRow(
            'Financing Activities',
            report.total_financing_cash_flow,
          ),
          Padding(
            padding: const EdgeInsets.symmetric(vertical: 8),
            child: Divider(color: Colors.green.shade900, thickness: 2),
          ),
          _buildMetricRow(
            'NET CASH CHANGE',
            report.cash_balance,
  ...
  Widget _buildActionButtons(CashFlowStatement report) {
    return Row(
      children: [
        Expanded(
  ...
  Future<void> _handleDownloadPdf(CashFlowStatement report) async {
    try {
      ...
      final fileName =
          'CashFlow_${report.report_id}_${DateTime.now().millisecondsSinceEpoch}.pdf';
  ...
  Future<void> _handleShare(CashFlowStatement report) async {
    try {
      final pdf = await _generatePdf(report);
      final directory = await getTemporaryDirectory();
      final file = File('${directory.path}/cash_flow_${report.report_id}.pdf');
      await file.writeAsBytes(await pdf.save());

      final params = ShareParams(
        files: [XFile(file.path)],
        text: 'Cash Flow Statement - ${report.report_id}',
      );
  ...
  Future<pw.Document> _generatePdf(CashFlowStatement report) async {
            ...
            children: [
              pw.Container(
                ...
                decoration: pw.BoxDecoration(
                  border: pw.Border(
                    bottom: pw.BorderSide(color: PdfColors.green900, width: 3),
                  ),
                ),
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
  ...
  Widget _buildReportFooter(CashFlowStatement report) {
    return Container(
